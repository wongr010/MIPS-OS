...
thread_create(const char *name) {
    ...
    for (i = t; i < TOTAL_PROCESSES; i++) {
        if (table[i].occupied == 0 && table[i].processthread == NULL) {
            
             table[i].occupied = 1;
            table[i].exitpermission=lock_create("exitperm");
            table[i].exitcv=cv_create("cv_exitcv");
          //  P(table[i].exitpermission); //this process is active, do not access the exit code
            thread->threadPID = i;
            table[i].PID = i;
            table[i].processthread = thread;
 ...
 * sure they don't 
 up again and interfere with the panic.
 */
static
void
thread_killall(void) {
...
thread_bootstrap(void) {
    ...
    //initialize menu locks
    menusleep=lock_create("menusleep");
...
void init_processtable() {
    ...
    for (i = 0; i < TOTAL_PROCESSES; i++) {
        table[i].processthread = NULL;
        table[i].occupied = 0; //struct process process_table[MAX_PROCESSES] declared in thread.h
        table[i].exited = 0;
...
void thread_onewakeup(const void *addr) {
