...
struct semaphore {
	char *name;
	volatile int count;
	int isOccupiedByMice;
	int isOccupiedByCat;
};

struct semaphore *sem_create(const char *name, int initial_count);
void              P(struct semaphore *); //decrement free resources (another one occupied)
void              V(struct semaphore *);//increment free resources (another one freed)
...
struct lock {
	...
	volatile int isAcquired;
	volatile struct thread *owner; //thread holding this lock
};
